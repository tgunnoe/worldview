...
wv.layers.modal = wv.layers.modal || function(models, ui, config) {
  ...
  var drawCategories = function() {
    ...
    _.each(layerCategories, function(layerCategory, layerCategoryKey) {
      ...
      if(category) {
        ...
        var $category = $('<div></div>').attr('data-sort', sortNumber)
        .addClass('layer-category layer-category-' + category.type)
        .attr('id', category.id);
        if (category.image) {
            ...
            _.each(measurement.categories, function(measurementCategory, measurementCategoryKey) {
              if(measurementCategory == category.id) {
                ...
                var $measurement = $('<a></a>')
                  ...
                  .attr('data-measurement', measurement.id)
                  .attr('title', category.title + ' - ' + measurement.title)
        ...
        var $filterButton = $('<input />').attr('type', 'radio').text(category.type);

        var $label = $('<label></label>').text(category.type);

        $filterButton.attr('id', 'button-filter-' + category.type).attr('data-filter', category.type).click(function(e) {
          $categories.isotope({
            filter: '.layer-category-' + category.type
          });
          $nav.find('.ui-button').removeClass('nav-selected');
          $("label[for=" + $(this).attr("id") + "]").addClass('nav-selected');
        });

        $label.attr('for', 'button-filter-' + category.type);
  ...
  var drawMeasurements = function(category, selectedMeasurement, selectedIndex) {
    ...
    // START FIND UNIQUE SOURCES HERE
    _.each(config.layers, function(layerVal, layerKey) {
      // Each layer with a measurement that equals the selected measurement.
    ...
    _.each(layerSources, function(aVal,aKey) {
      ...
      var $sourceLink = $('<a></a>').text(source.title).attr('href', '#' + measurement.id + '-' + source.id);

      $sourceTab.append($sourceLink);
      $sourceTabs.append($sourceTab);

      var $sourceContent = $('<div></div>').attr('id', measurement.id + '-' + source.id);
      ...
      _.each(config.layers, function(layer, layerName) {
        ...
        if(layer.hasOwnProperty('sources')) {
          if (Object.values(layer.sources).indexOf(aVal) > -1) {
            // console.log(layer);
